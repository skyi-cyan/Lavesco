...
2. **Android 패키지 이름** 입력
   - 예: `com.lavesco.app`
   - ⚠️ 이 값은 나중에 변경할 수 없으므로 신중하게 결정하세요
3. **앱 닉네임** 입력 (선택): `Lavesco`
4. **디버그 서명 인증서 SHA-1** (선택, 나중에 추가 가능)
5. **"앱 등록"** 클릭

#### 1.2. google-services.json 다운로드
...
`mobile/android/build.gradle` 파일 확인:
...
2. **iOS 번들 ID** 입력
   - 예: `com.lavesco.app`
   - ⚠️ Android와 동일한 패키지 이름 사용 권장
3. **앱 닉네임** 입력 (선택): `Lavesco`
4. **App Store ID** (선택, 나중에 추가 가능)
5. **"앱 등록"** 클릭

#### 2.2. GoogleService-Info.plist 다운로드

1. **"GoogleService-Info.plist 다운로드"** 클릭
2. 파일을 `mobile/ios/Runner/` 디렉토리에 복사
...
Xcode에서 `mobile/ios/Runner.xcworkspace`를 열고:

1. **Runner** 프로젝트 선택
2. **Build Phases** 탭
3. **"+"** 버튼 클릭 > **"New Run Script Phase"**
4. 스크립트 추가:

```bash
"${PODS_ROOT}/GoogleService-Info.plist"
```

또는 수동으로 `GoogleService-Info.plist`를 Xcode 프로젝트에 추가합니다.

### 3단계: FlutterFire CLI 설정

#### 3.1. FlutterFire CLI 설치
...
3. 자동으로 `lib/firebase_options.dart` 파일 생성됨

#### 3.3. 생성된 파일 확인

`mobile/lib/firebase_options.dart` 파일이 생성되었는지 확인합니다.

### 4단계: Flutter 앱에서 Firebase 초기화

`mobile/lib/main.dart` 파일을 업데이트:

```dart
import 'package:flutter/material.dart';
import 'package:flutter_riverpod/flutter_riverpod.dart';
import 'package:firebase_core/firebase_core.dart';
import 'firebase_options.dart'; // 생성된 파일 import
...
앱이 정상적으로 실행되고 Firebase 연결이 확인되면 성공입니다.
...
`mobile/android/app/src/main/AndroidManifest.xml`에 추가:
...
`mobile/ios/Runner/Info.plist`에 추가:
...
**해결 방법**:
1. 파일이 `mobile/android/app/` 디렉토리에 있는지 확인
2. `mobile/android/app/build.gradle`에 `apply plugin: 'com.google.gms.google-services'` 추가 확인
...
**해결 방법**:
1. 파일이 `mobile/ios/Runner/` 디렉토리에 있는지 확인
